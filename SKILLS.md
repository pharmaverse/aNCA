---
name: agent-workflow
description: How to work with GitHub Copilot agents (Ona) in the aNCA development workflow. Use when asking agents to review PRs, fix issues, or handle tasks.
---
...
3. Identify style issues, missing tests, and documentation gaps
4. Flag findings with severity levels
5. Provide information about the findings (question for clarification, important fix to do, potential bug), with suggestions on what the developer should do next.

Agent will NOT:
- Post any comments on the PR or push anything, unless the user specifies it.
- Comment on CI/CD failure status.
- Flag minor stylistic nitpicks (e.g., whitespace, trailing commas) that do not impact readability or execution.
- Comment on the changes that have been merged from the main and that are not covered in the PR.
...
Agent will:
1. Address each specified finding
2. Push commits (keeping fixes separate where possible) with user approval
3. Post a PR comment summarizing changes with user approval
...
2. Identify which check failed (lint, tests, etc.)
3. Suggest fixes
...
Agent will:
1. Create test file `tests/testthat/test-x.R`
2. Match existing test patterns
3. Include descriptive test names and assertions
...
Agent will:
1. Add/update `@param`, `@returns`, `@examples`
2. Regenerate man pages (if R available)
...
Agent will:
1. Map data flow through the code
2. Identify which functions are called
3. Show the output at each step
...
2. Update all calls to the function
...
❌ `"Fix everything"` — too vague, agent will guess
✅ `"Fix findings 1, 3, 5"` — clear and reviewable

❌ Skip testing Shiny changes — agent cannot see the UI
✅ Test yourself after agent edits Shiny code

❌ Ask `"Can you?"` or `"Should I?"` — use direct requests
✅ `"Add tests for X"` or `"Rename X to Y"`

❌ Assume agent remembers your last 10 questions
✅ Include PR number: `"Review PR #123"` every time
