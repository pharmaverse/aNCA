...
describe("flexible_violinboxplot", {
  it("creates a simple plot with minimal arguments", {
    simple_plot <- flexible_violinboxplot(
      ...
      columns_to_hover = c("DOSEA", "USUBJID", "AVISIT", "PARAM"),
      box = TRUE,
      plotly = FALSE
    )

    expect_equal(simple_plot$labels$x, "DOSEA")
    expect_equal(simple_plot$labels$colour, "AVISIT")
  ...
  it("creates a plot with additional xvars", {
    ...
    expect_equal(xvars_plot$labels$colour, "AVISIT")
  ...
  it("creates a plot with additional colorvars", {
    ...
    expect_equal(colorvars_plot$labels$colour, "AVISIT, PARAM")
  ...
  it("creates a plot with additional varvalstofilter", {
    varvalstofilter_plot <- flexible_violinboxplot(
      ...
      xvars = "DOSEA",
      colorvars = "AVISIT",
      varvalstofilter = c("USUBJID: 1", "USUBJID: 2", "USUBJID: 6", "AVISIT: 1"),
      columns_to_hover = c("DOSEA", "USUBJID", "AVISIT", "PARAM"),
      box = TRUE,
      plotly = FALSE
    )
  ...
  it("creates a violin plot when box = FALSE", {
    ...
    expect_equal(violin_plot$labels$colour, "AVISIT")
  ...
  it("handles missing data gracefully", {
    ...
    expect_equal(missing_plot$labels$colour, "AVISIT")
