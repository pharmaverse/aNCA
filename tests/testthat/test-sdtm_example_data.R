# Shared skip + data loading for all SDTM example tests
.load_sdtm_test_data <- function(envir = parent.frame()) {
  skip_if_not(
    file.exists(system.file("data", "dm_example.rda", package = "aNCA")),
    message = "SDTM example .rda files not yet generated"
  )
  data("adnca_example", package = "aNCA", envir = envir)
  data("dm_example", package = "aNCA", envir = envir)
  data("pc_example", package = "aNCA", envir = envir)
  data("ex_example", package = "aNCA", envir = envir)
}

describe("SDTM example: dm_example", {
  .load_sdtm_test_data()
  ...
  it("has valid ISO 8601 datetimes in RFXSTDTC", {
    parsed <- as.POSIXct(dm_example$RFXSTDTC,
                         format = "%Y-%m-%dT%H:%M:%S", tz = "UTC")
    expect_false(any(is.na(parsed)))
  })
})

describe("SDTM example: pc_example", {
  .load_sdtm_test_data()
  ...
  it("has valid ISO 8601 datetimes in PCDTC", {
    parsed <- as.POSIXct(pc_example$PCDTC,
                         format = "%Y-%m-%dT%H:%M:%S", tz = "UTC")
    expect_false(any(is.na(parsed)))
  })
...
describe("SDTM example: timing consistency", {
  ...
  it("preserves dose amounts from adnca_example", {
    ...
    merged <- merge(adnca_doses, ex_doses,
                    by = c("USUBJID", "DOSETRT", "DOSEA"))
...
describe("SDTM example: ex_example", {
  .load_sdtm_test_data()
  ...
  it("has the same subjects as adnca_example", {
    expect_setequal(unique(ex_example$USUBJID),
                    unique(adnca_example$USUBJID))
  })

  it("has valid ISO 8601 datetimes in EXSTDTC", {
    parsed <- as.POSIXct(ex_example$EXSTDTC,
                         format = "%Y-%m-%dT%H:%M:%S", tz = "UTC")
    expect_false(any(is.na(parsed)))
  })

  it("has EXENDTC >= EXSTDTC for all rows", {
    start <- as.POSIXct(ex_example$EXSTDTC,
                        format = "%Y-%m-%dT%H:%M:%S", tz = "UTC")
    end <- as.POSIXct(ex_example$EXENDTC,
                      format = "%Y-%m-%dT%H:%M:%S", tz = "UTC")
  ...
  it("has EXELTM starting at PT0H for each subject's first dose", {
    first_eltm <- tapply(ex_example$EXELTM, ex_example$USUBJID,
                         function(x) x[1])
  ...
  it("has treatment names matching adnca_example$DOSETRT", {
    expect_setequal(unique(ex_example$EXTRT),
                    unique(adnca_example$DOSETRT))
  })
})
