...
describe("faceted_qc_plot", {
  ...
  it("returns a ggplot object when as.plotly is FALSE", {
    p <- faceted_qc_plot(
  ...
  it("returns a plotly object when as.plotly is TRUE", {
    p <- faceted_qc_plot(
  ...
  it("appends units to labels correctly when units are unique", {
    ...
    # Assumes get_label returns "Actual_time" and the unit is appended.
  ...
  it("omits units from labels when units are not unique", {
    ...
    # The label should not contain parentheses if units are omitted.
