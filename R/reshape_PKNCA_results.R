...
reshape_PKNCA_results <- function(resNCA) {
  ...
  dict_pttestcd_with_units <- resNCA$result %>%
  ...
  if (any(resNCA$result$PPTESTCD == "aucint.last")) {
    ...
    all_AUCs <- merge(infinite_AUCs_with_lambda_details, interval_AUCs, all = TRUE)
  } else {
    all_AUCs <- infinite_AUCs_with_lambda_details
  }
