...
#'
#' @param data A data frame. The pivot-wider NCA results containing subject data from `PKNCAresults` object.
#' @param nca_intervals A data frame. The intervals object from the PKNCA result (e.g., `res$data$intervals`).
...
apply_results_flags <- function(data, nca_intervals, group_cols, flag_settings) {
  ...
  if (length(flag_cols) > 0) {
    ...
    flagged_data <- flagged_data %>%
      ...
      rowwise() %>%
      mutate(
        na_msg_vec = list(purrr::map2_chr(flag_cols, flag_params_pknca[valid_indices], function(d_col, f_col) {
