...
pc_to_PKNCAconc <- function(pc, ex, dm = NULL, metabolites = NULL) { # nolint: object_name_linter
  ...
  # --- NCA exclusion column ----------------------------------------------------
  # If PCREASND is mapped, use it as the exclusion reason (same role as
  # NCAwXRS in the ADNCA path). Otherwise default to empty string.
  if ("PCREASND" %in% names(conc_data)) {
    conc_data$nca_exclude <- ifelse(
      is.na(conc_data$PCREASND), "", trimws(conc_data$PCREASND)
    )
  } else {
