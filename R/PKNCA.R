...
#'     PKSUMXF = "Y" and the reason is stored in PKSUMXRS so
...
add_exclusion_reasons <- function(pknca_data, exclusion_list) {
  ...
  # Initialise PKSUMXF and PKSUMXRS if not present
  if (!"PKSUMXF" %in% names(pknca_data$conc$data)) {
    pknca_data$conc$data$PKSUMXF <- ""
  }
  if (!"PKSUMXRS" %in% names(pknca_data$conc$data)) {
    pknca_data$conc$data$PKSUMXRS <- ""
  }
  ...
  for (excl in exclusion_list) {
    ...
    if (isTRUE(excl$exclude_tlg)) {
      ...
      pknca_data$conc$data$PKSUMXRS[rows] <- ifelse(
        pknca_data$conc$data$PKSUMXRS[rows] == "",
        reason,
        paste0(
          pknca_data$conc$data$PKSUMXRS[rows],
          "; ", reason
