...
#' @param selected_usubjids A character vector of selected unique subject identifiers (USUBJIDs) to be included in the plot.
#' @param colorby_var A character string specifying the variable by which to color the lines in the plot.
#' @param time_scale A character string specifying the time scale. Options are "By Cycle" or other values.
#' @param xaxis_scale A character string specifying the x-axis scale. Options are "Log" or other values.
#' @param cycle A character string or numeric value specifying the cycle to filter by when `time_scale` is "By Cycle". Default is NULL.
#'
...
general_lineplot <- function(data, 
  ...
  plt <- g_ipp(df = preprocessed_data,
               xvar = "AFRLT",
               yvar = "AVAL",
               ...
               subtitle = paste0("Subjects: ", paste(unique(preprocessed_data$USUBJID), collapse = ", "),
    ...
    if (xaxis_scale == "Log"){
      plt <- plt + scale_y_log10(breaks = c(0.001, 0.01, 0.1, 1, 10), label = c(0.001, 0.01, 0.1, 1, 10)) +
