...
#' extracted from `res_nca`. It computes individual bioavailability where IV and EX data are available
#' for a subject. If IV data is missing, it estimates bioavailability using the mean IV values for that grouping.
#'
#' @details
...
#' 
#' @param res_nca A list containing non-compartmental analysis (NCA) results, including concentration
#'   and dose data.
#' @param selected_aucs A character vector of selected AUC variables (e.g., `c("f_aucinf.obs", "f_auclast")`).
#'   The function automatically removes the "f_" prefix to match with `PPTESTCD` values in the dataset.
#'
#' @returns A data frame with calculated bioavailability values (`f_aucinf`, `f_auclast`, etc.)
#'   for individual subjects where IV data is available. If IV data is missing, it estimates bioavailability
#'   using the mean IV AUC for that grouping.
#'
#' @export 
