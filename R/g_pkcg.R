...
pkcg03 <- function(
  ...
  summary_settings <- list(
    Mean_ci = list(mid = "mean", interval = "mean_ci", whiskers =
                     c("mean_ci_lwr", "mean_ci_upr")),
    Mean_sdi = list(mid = "mean", interval = "mean_sdi", whiskers =
                      c("mean_sdi_lwr", "mean_sdi_upr")),
    Mean_se = list(mid = "mean", interval = "mean_sei", whiskers =
                     c("mean_sei_lwr", "mean_sei_upr")),
    Median_ci = list(mid = "median", interval = "median_ci", whiskers =
                       c("median_ci_lwr", "median_sci_upr"))
  )
  ...
  plots <- lapply(plot_ids, function(id_val) {
    ...
    group_by(
      .data[[mean_group_var]], .data[[xvar]]) %>%
      summarise(
        # Count number of samples for each timepoint by group
    ...
    title <- paste0(
      method_label, " ", generate_title_mean(plot_data, title, scale, studyid, mean_group_var)
    )
    ...
    if (!requireNamespace("scales", quietly = FALSE))
      ...
      # Create SBS version of data and plot
    sbs_data  <- bind_rows(
      plot$data %>% dplyr::mutate(view = "Linear view"),
      plot$data %>% dplyr::mutate(view = "Semilogarithmic view (Log10)")
    ) %>%
      dplyr::mutate(
        !!sym(aval_stat) := ifelse(
