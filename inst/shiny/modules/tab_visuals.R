...
tab_visuals_ui <- function(id) {
  ...
  navset_card_pill(
    ...
    nav_panel("Descriptive Statistics",
              summary_statistics_ui(ns("summary_statistics"))
    ),
    nav_panel("Boxplot",
              layout_sidebar(
                sidebar = sidebar(
...
tab_visuals_server <- function(id, data, grouping_vars, res_nca) {
  moduleServer(id, function(input, output, session) {
    ...
    output$cycle_select <- renderUI({
      ...
      y <- data() %>% filter(ANALYTE %in% input$generalplot_analyte) %>% pull(DOSNO) %>% unique()
      selectInput(ns("cycles"), "Choose the cycle :", choices = sort(y), multiple = TRUE, selected = y[1])
    })
    ...
    observeEvent(input$analyte_mean, {
      req(data())
      cycle_choices <- data() %>% filter(ANALYTE %in% input$analyte_mean) %>% pull(DOSNO) %>% unique()
      updateSelectInput(session, "cycles_mean", choices = sort(cycle_choices))
    })
    ...
    boxplotdata <- reactive({
      ...
      left_join(
        res_nca()$result,
        res_nca()$data$conc$data %>% distinct(across(all_of(group_columns)), .keep_all = TRUE),
        by = group_columns,
        keep = FALSE
      ) %>%
    ...
    observeEvent(boxplotdata(), {
      ...
      param_choices <- boxplotdata() %>% pull(PPTESTCD) %>% unique()
      updatePickerInput(session, "selected_param_boxplot", choices = param_choices)
      
      # Update the selected_xvars_boxplot picker input
      xvar_choices <- intersect(names(boxplotdata()), names(data()))
      updatePickerInput(session, "selected_xvars_boxplot", choices = xvar_choices, selected = "DOSEA")
      
      # Update the selected_colorvars_boxplot picker input
      colorvar_choices <- intersect(names(boxplotdata()), names(data()))
      updatePickerInput(session, "selected_colorvars_boxplot", choices = colorvar_choices, selected = "DOSNO")
    })
