...
non_nca_ratio_server <- function(id, data, grouping_vars) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$add_pair, {
      ...
      observeEvent(input[[spec1_id]], { selections[[spec1_id]] <- input[[spec1_id]] }, ignoreNULL = FALSE)
      observeEvent(input[[spec2_id]], { selections[[spec2_id]] <- input[[spec2_id]] }, ignoreNULL = FALSE)
