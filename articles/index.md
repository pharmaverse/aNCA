# Articles

### All vignettes

- [Adding new TLGs to the
  package](https://pharmaverse.github.io/aNCA/articles/adding_tlg.md):

  Learn how to add a new TLG to the package.

- [aNCA Shiny App User
  Guide](https://pharmaverse.github.io/aNCA/articles/aNCA.md):

  Learn how to get started with the basics of aNCA Shiny application.

- [What is
  NCA?](https://pharmaverse.github.io/aNCA/articles/background.md):

  Background information on Non-Compartmental Analysis (NCA) and its
  applications in pharmacokinetics.

- [Contributing to
  aNCA](https://pharmaverse.github.io/aNCA/articles/contributing.md):

  Learn how to contribute to the development of the aNCA package

- [Design &
  Architecture](https://pharmaverse.github.io/aNCA/articles/design.md):

  This document captures the high-level architecture, data flow, and key
  design decisions for the aNCA package and its embedded Shiny
  application.

- [FAQs - Frequently Asked
  Questions](https://pharmaverse.github.io/aNCA/articles/FAQs.md):

  This document provides answers to frequently asked questions about the
  aNCA package and Shiny application.

- [Governance
  model](https://pharmaverse.github.io/aNCA/articles/governance.md):

  This document outlines the governance model for the aNCA project,
  detailing roles, responsibilities, and decision-making processes to
  ensure effective management and collaboration within the project.

- [aNCA Manual](https://pharmaverse.github.io/aNCA/articles/manual.md):

  User guide and technical reference for aNCA: data, workflow, settings,
  and outputs.

- [Roadmap
  Goals](https://pharmaverse.github.io/aNCA/articles/roadmap.md):

  This document captures the high-level goals planned for the aNCA
  project in the next year.

- [Validation
  Plan](https://pharmaverse.github.io/aNCA/articles/validations.md):
