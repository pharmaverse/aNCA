...
- [Column mapping gotchas](#column-mapping-gotchas)
...
- [Building the pkgdown site locally](#building-the-pkgdown-site-locally)
...
The app accepts common formats: CSV, RDS, XPT (SAS transport), parquet and Excel. The preferred layout is the ADNCA/ADaM-like table used throughout the package (see the user guide). Key points:
...
- Precompute a column in your dataset that selects nominal or actual time per-row based on your deviation rule (for example, create a `USE_TIME` column: if |AFRLT - NFRLT| &lt; threshold then NFRLT else AFRLT) and map that column as the time variable in the app.
...
Duplicates are rows with the same subject/param/time and can break slope selection and NCA calculations. The Mapping UI detects potentially blocking duplicates and opens a modal allowing you to select which rows to KEEP. Recommended workflow:
...
3. If you prefer deterministic rules (keep first, last, non-missing), let us know — we can add an auto-resolve option.
...
You can see our current validations and future plan in the [validations page]()
</details>
...
The user section in [CONTRIBUTING.md](aNCA/CONTRIBUTING.md) specifies how to act in each of these cases.
...
We still did not answered your questions? Check at our [GitHub discussions](https://github.com/pharmaverse/aNCA/discussions) and if it is still not there feel free to open a Q&A topic! 
