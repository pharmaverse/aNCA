...
configured in **NCA > Setup > Settings > Ratio Calculations**.
...
**PPTESTCD:** `MR{param}` — e.g. `MRCMAX`, `MRAUCLST`
...
**PPTESTCD:** `AR{param}` — e.g. `ARCMAX`, `ARAUCLST`
...
| Test Parameter | `CMAX` |
| Ref Parameter | `CMAX` |
| Ref Groups | `ATPTREF: 1` (single dose profile) |
| Test Groups | `(all other levels)` or `ATPTREF: 5` |
| Adj. Factor | 1 |
...
| Field | Value |
|---|---|
| Test Parameter | `AUCLST` (or `AUCIFOB`) |
| Ref Parameter | `AUCLST` (or `AUCIFOB`) |
...
- If the parameters are already dose-normalized (e.g. `AUCLST/D`), set the
