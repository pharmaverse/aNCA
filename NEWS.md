...
* Fixed ratio calculation failures for non-ATPTREF contrast variables (e.g. RACE, SEX, AGE, COHORT) when `aggregate_subject = "no"` and profiles have different absolute dosing times. `start`/`end` and `USUBJID` are now dropped from match columns only for variables that are constant within subjects (true between-subject variables); within-subject variables like `PARAM` keep them so test/ref rows pair per subject and dose (#1286)
