...
### Export & Output
* Rendered TLGs have their own format controls in the export dialog, one for graphs and one for tables and listings, defaulting to PDF and XLSX. Graphs offer PNG, PDF and HTML; tables and listings offer CSV, XLSX and PDF, the last as a fixed paginated rendering for filing, split across pages by column as well as by row so a wide summary table is never clipped. They previously shared the CDISC dataset formats, where the defaults suited neither: HTML was on, so a large order silently produced hundreds of MB of interactive widgets, while XLSX was off, so tables could only ever come out as CSV unless the user went looking. Each format now gets its own folder (`Graphs/png/`, `Tables/xlsx/`) and every TLG its own subdirectory, so formats are no longer interleaved in one listing (#1344)
* Export every rendered TLG: the "Export as ZIP" button now offers Tables, Listings and Graphs alongside the other results, once a TLG order has been submitted. They are written to `TLGs/` as `Graphs/`, `Tables/` and `Listings/` plus a `manifest.csv`, named after the catalog entry and the split key (e.g. `TLGs/Graphs/pkcg01_lin/DrugA.png`). A TLG that splits gets its own folder, and each format is kept in its own folder. Anything that groups an output into a single document does so per catalog entry rather than per plot or per split: PDF and HTML give one document holding every plot, XLSX one workbook with a sheet per split, and the table PDF one paginated document with a section per split. So a per-subject output is a single file to page through instead of a hundred, which for HTML is also the difference between an archive of a few MB and one of several hundred, since every widget written on its own inlines its own copy of the same JavaScript. The export reflects the sidebar options currently applied to each output, and a TLG that could not be produced is recorded in the manifest instead of failing the whole download (#1344)
