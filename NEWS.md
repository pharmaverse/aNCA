...
* The terminal-slope (half-life) family (`LAMZ`, `LAMZHL`, `R2`, `R2ADJ`, `LAMZLL`, `LAMZUL`, `LAMZNPT`, `LAMZSPN`, `CLSTP`) now keeps its imputed values so it reconciles with the AUCinf and clearance results derived from the same imputed slope. Previously imputation was stripped from these parameters as if they were observational, so the reported slope came from a different, non-imputed interval row than the AUCinf/CL that consumed it. The family is now derived from parameter dependencies rather than the metadata category (#1443)
