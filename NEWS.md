...
* Text entered in a TLG sidebar box (Title, Subtitle, Footnote, axis labels) survives re-submitting the order. Re-submitting rebuilds the module UI, which re-ran the code that builds the option widgets and pushed the declared default back to the client, discarding whatever had been typed; the current value is now carried forward instead. "Reset to defaults" sets the declared default explicitly rather than relying on the value the widget was created with (#1476)
...
* Every one of the 33 TLG catalog entries now offers Title, Subtitle and Footnote in the right sidebar, pre-filled with the reference title wording from the published TLG catalog. Footnotes describe what aNCA actually renders rather than copying the catalog's `NE: Not Estimable`, which names a code these tables never print (non-estimable cells are left empty). The nine summary tables gained the three inputs outright: they had no title mechanism at all, so `t_pkct01`, `t_pkpt03_col`, `t_pkpt07_norm`, `t_pkpt08_uri` and `t_pkpt11_gmr` take `title`, `subtitle` and `footnote` arguments and the rendered table carries them above and below the grid. `l_pkcl02_uri` binds `title` as a named parameter, so the urine listing title is editable instead of hardcoded. The `pkcg01`, `pkcg02` and `pkcg03` plots keep generating their title and subtitle per render, since those carry the scale wording, the cohort and the "N=" subject count that a static string cannot reproduce. The six `pkpg` parameter plots generate subtitles from each plot's analyte, specimen, visit and parameter/unit pairs, with custom subtitle text taking precedence (#1476)
* Table subtitles follow the chosen split variables rather than naming a fixed analyte, so changing "Split tables by" relabels the subtitle to match (#1476)
