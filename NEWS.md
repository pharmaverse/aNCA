...
* PK parameter listings no longer show a blank cell for a subject that has a value. Where several rows collapse into one cell — a multi-dose ADPP with one row per dose profile — the first was taken even when missing, so the listing contradicted the summary table. The first present value is used instead (#1429)
