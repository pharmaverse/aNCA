...
* Individual (`pkcg01`) and combined (`pkcg02`) concentration plots return a named list again, so each plot is captioned with its group the way the other split graphs are. The names were taken from the ungrouped input, where the internal `id_plot` grouping column does not exist, so `setNames()` silently received `NULL` and dropped them (#1344)
...
### Export & Output
* Export every rendered TLG: the "Export as ZIP" dialog now offers Tables, Listings and Graphs alongside the other results, from the moment a TLG order has been submitted and without waiting for NCA. Graphs get their own format control and tables and listings another, defaulting to PDF and XLSX: graphs offer PNG, PDF and HTML, tables and listings offer CSV, XLSX and PDF, the last a fixed paginated rendering for filing that splits across pages by column as well as by row so a wide summary table is never clipped. Outputs are written under `TLGs/` as `Graphs/`, `Tables/` and `Listings/`, one folder per format and per TLG (e.g. `TLGs/Graphs/png/pkcg01_lin/S1.png`), alongside a `manifest.csv` recording every output with its path and status. Anything collected into a single document is collected per catalog entry rather than per plot or per split, so a per-subject output is one file to page through instead of a hundred: PDF and HTML give one document holding every plot, XLSX one workbook with a sheet per split, and the table PDF one paginated document with a section per split. The export reflects the sidebar options currently applied to each output, and a TLG that could not be produced is recorded in the manifest instead of failing the whole download (#1344)
