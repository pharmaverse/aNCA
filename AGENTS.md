...
- **PR title**: Use `TYPE: concise outcome (#issue)` so the title states the primary change in a few words and ends with its issue number. Use an accurate uppercase type such as `FEATURE`, `FIX`, `REFACTOR`, `SKILL`, `DOCS`, `CI`, or `MAINT`; for example, `FIX: Prevent partial ZIP exports (#1452)`.
